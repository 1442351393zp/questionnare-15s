							...
							<div class="question-list">
							    <c:forEach items="${pages.subjectList}" var="subject" varStatus="sub">
									...
									<div class="question-item-content">
										...
										<c:forEach items="${answerList}" var="opt">
											 <c:if test="${opt.optionsId == options.optionsId}">checked</c:if>
										</c:forEach>
										name="options${subject.subNo}" value="${options.optionsId}">
								    ...
								    <div class="question-item">
									    <div class="question-item-title">
										    ...
										    <input name="subject" type="hidden" value="${subject.subjectId}">
										    <input name="pageId" type="hidden" value="${pages.pageId}" >
											<input name="subjectType" type="hidden" value="${subject.subjectType}" >
                                            <c:if test="${reelVo.showNo == 0}"><span>${subject.subNo}</span></c:if>
                                            <c:if test="${reelVo.showNo == 1}"><span></span></c:if>
											<span>${subject.topic}</span>
											<span>*</span>
									    ...
									    <c:forEach items="${subject.optionsList}" var="options">
											...
											<c:when test="${options.isMultipleChoice == '0'}">
												<div class="checkbox-primary checkbox-btn" style="display: flex;">
													...
													/>
                                                    <c:forEach items="${answerList}" var="opt">
                                                        <c:if test="${opt.optionsId == options.optionsId}">
                                                            <c:choose>
                                                                <c:when test="${opt.textAnswer !=''&& opt.textAnswer !=null}">
                                                                    <label class="wrap-other">
                                                                        <div class="span-other" >其他：</div>
                                                                        <label class="textarea-other" for="${options.optionsId}">${opt.textAnswer}</label>
                                                                    </label>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <label class="wrap-other">
                                                                        <div class="span-other" >其他：</div>
                                                                        <label class="textarea-other" for="${options.optionsId}"></label>
                                                                    </label>
                                                                </c:otherwise>
														...
														<div class="box-single__line" style="margin-bottom:10px;">
															<div class="fieldset"><div class="label"></div>
																...
																<div class="editable optionIpt single-line-text"
																	...
																	<c:forEach items="${answerList}" var="opt2">
																		<c:if test="${opt2.optionsId == options.optionsId}">${opt2.textAnswer}</c:if>
																	</c:forEach>
																</div>
															</div>
														</div>
